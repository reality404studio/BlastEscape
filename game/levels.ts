...
export const LEVELS: readonly LevelDefinition[] = [
  ...
  },
  {
    id: 'level-24',
    name: 'LEVEL 24',
    subtitle: 'TEMPER CIRCUIT',
    hint: 'Build the blue route to B1. On the locked carriage, cross red to wake B2—then return left.',
    intent: {
      primaryRoute: 'START -> coolant -> freeze intake -> cold-lock carriage -> B1 -> locked carriage -> furnace replaces cold with heat -> power dormant B2 -> right launch stop -> B2 -> steer left -> upper temper deck -> EXIT',
      launchJobs: [
        { bomb: 'B1', job: 'Transfer from the lower circuit onto the cold-stabilized carriage.' },
        { bomb: 'B2', job: 'After heat ignition, launch from the carriage right stop and return left to the upper deck.' },
      ],
      landingWindows: [
        { target: 'locked transfer carriage', kind: 'wide', description: 'Cold fixes the broad carriage at one readable receiving dock.' },
        { target: 'upper temper deck', kind: 'medium', description: 'B2 needs a deliberate leftward return, with a forgiving inspection deck.' },
      ],
      timingWindows: [
        { target: 'cold intake and carriage lock', kind: 'expiring', description: 'Both cold-created route pieces cover the B1 transfer and carriage crossing.' },
        { target: 'heat-powered B2', kind: 'commit', description: 'Ignition leaves time to reach the marked right stop before the dormant charge fires.' },
      ],
      recovery: { kind: 'recoverable', description: 'Stable lower floors permit circuit setup retries; missing the carriage or upper deck falls into the exposed void.' },
      masteryShortcut: null,
      newConcept: 'None; the cold-built route hands control to a heat-powered second launch after carriage commitment.',
      recombinedSkills: ['temporary ice', 'cold machinery lock', 'B1 transfer', 'state replacement', 'heat charge reactivation', 'prepared B2 return'],
      targetFirstClearSeconds: { min: 180, max: 330 },
    },
    validation: {
      requiredBlastHits: ['B1', 'B2'],
      requiredInteractions: ['temper-intake-freeze', 'temper-carriage-lock', 'temper-ignition'],
      requiredStates: ['cold', 'heat'],
      simplePoliciesMustFail: ['hold-left', 'neutral', 'hold-right'],
      noisyHumanProfiles: [
        { jitterMilliseconds: 80, samples: 100, minimumClearRate: 0.7 },
    ...
    bombs: [
      { x: 570, y: 532, delay: -1.6, label: 'B1' },
      {
        ...
        delay: -3.2,
        label: 'B2',
        reactivatedByInteractionId: 'temper-ignition',
      },
    ],
    exit: { x: 730, y: 156, w: 64, h: 64 },
    pit: { x: 650, y: 470, w: 292, h: 130 },
    movingPlatform: {
      ...
      speed: 150,
      phase: 0.45,
      stabilizedByInteractionId: 'temper-carriage-lock',
      stabilizedX: 650,
    },
    traversalStateSources: [
      {
    ...
    waterHazards: [
      {
        id: 'temper-intake-pool',
        rect: { x: 300, y: 550, w: 180, h: 50 },
        frozenByInteractionId: 'temper-intake-freeze',
      },
    ],
  },
  {
